...
const RidgelinePlot: React.FC<RidgelinePlotProps> = ({ onSelectPosition, timeframe }) => {
  const svgRef = useRef<SVGSVGElement | null>(null);
  const title = timeframe === 'weekly'
  ...
  useEffect(() => {
    ...
    const csvFile = timeframe === 'weekly' 
        ...
        svg.selectAll('areas')
          ...
          .attr(
            ...
            d3.line<[number, number]>()
              .curve(d3.curveBasis)
              .x((point) => x(point[0]))
              .y((point) => Math.min(0, -point[1] * Y_SCALING_FACTOR)) // Negative for upward ridgeline
          )
